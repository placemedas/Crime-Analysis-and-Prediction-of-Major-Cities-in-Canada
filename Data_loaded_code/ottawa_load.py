...
spark = SparkSession.builder.appName('Load Ottawa data') \
...
def main():
	...
	url = 'https://moto.data.socrata.com/resource/uacw-px76.json?$limit=200000'#url to retrieve Ottawa dataset from Socrata

	urllib.request.urlretrieve(url, "ottawa.json")#make a request to fetch the required resource


	#Deleting previous ottawa.json datasets
	hdfs_path = "/user/klnu/ottawa.json"
	...
	file_path = "/home/klnu/ottawa.json"
	hdfs_path = os.path.join(os.sep, 'user', 'klnu', "ottawa.json")
	...
	crime_data = spark.read.json("ottawa.json", multiLine=True)
	...
	#Appending the base_table(table where crime data for other cities are stored) with ottawa dataframe
